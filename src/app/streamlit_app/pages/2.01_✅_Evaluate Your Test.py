...
# Initialize the structure to store the evaluation results
evaluation_results = {}
...
for student in students:
    ...
    for competency in current_test_competencies:
        competency_type = competency.get_competency_type().type
        # Display radio buttons for each competency evaluation
        evaluation_option = st.radio(
            label=competency_type,
            options=[
            ...
            key=f"{student.id}_{competency.id}"
        )
        student_evaluations[competency.id] = evaluation_option
    
    evaluation_results[student.id] = student_evaluations

def save_data():
    for student_id, evaluations in evaluation_results.items():
        print(f"Saving evaluations for student {student_id}: {evaluations}")
        for competency_id, mark in evaluations.items():
            print(f"Saving evaluation for competency {competency_id}: {mark[0]}")
            # Assuming there's a function to add the evaluation to the student
            add_test_evaluation_to_student(student_id, competency_id, mark[0], "")
    # Assuming there's a modal or notification to close or show after saving
    # modal.close()
